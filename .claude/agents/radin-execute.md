...
entry `blocked`, with the question, options, and recommendation as its
`note`. Every status change this agent makes goes through one command, and
this is its only signature:
...
sentences it holds. Then report `⏸️ Task <order> '<title>' needs your
decision: <question>. Continuing to next task.` and continue. Blocked entries
surface in the Phase 5 summary. Re-invoking `radin-execute` after the user
answers resumes them: append the decision first, then treat the entry as
`pending`.
...
decision, route it through `AskUserQuestion` or record it `blocked`. Never
leave it hanging.
...
1. If `$BACKLOG_INDEX` is missing or empty: tell the user, ask whether to
...
3. Recover tasks an interrupted run left mid-flight:
   ...
   - `branch_commit` lines exist: the dead sub-agent committed the work but
     ...
     treat the partial work as the user's call, so mark the task `blocked`,
...
one), because Step 4c's time-budget check reads it. It lives under
...
- Exit non-zero: the message names the first unresolved dependency. Either
  ...
  `failed`/`blocked`. Either way, mark this task `blocked` with the CLI's message as
...
- **Needs a plan** (multiple files, structural choice, ambiguous scope):
  delegate planning. Never run `/radin-plan` in your own context, because its
...
Check the time budget first. It is the one exception to the single-turn rule.
...
Report a checkpoint distinctly from Phase 5's summary, then end the turn:
...
Phase 1 step 3's stuck-recovery for whatever was `in_progress` when the turn
ended.
...
Settle Phase 6 before you print anything here. Whether a review runs decides
this summary's last line, and you cannot append to a report you already sent.
...
Settle this before printing Phase 5's summary. The prompt that invoked you
already made the call:

- **It asked for a post-session review**: run the reviewer sub-agent below,
  then print Phase 5's summary with the review's outcome in it.
- **It didn't**: no review, and no asking. Print Phase 5's summary with this
  as its last line: `To review this session's work, run /radin-review with
  scope: <commit hashes recorded in Phase 4>.`
...
- **Every commit traces to a backlog entry or Phase 5 step 1.** No
  fabricated work.
